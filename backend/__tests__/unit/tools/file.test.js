...
describe('list_files tool', () => {
    ...
    test('showHidden=true includes dot-files', async () => {
        fs.existsSync.mockReturnValue(true);
        fs.statSync.mockReturnValue(fakeDirStat());
        fs.readdirSync.mockReturnValue([{ name: '.env', isDirectory: () => false, isFile: () => true }]);
...
describe('create_file tool', () => {
    test('creates new file', async () => {
        ...
        expect(fs.writeFileSync).toHaveBeenCalledWith(expect.any(String), 'Hello World', 'utf-8');
    });
