...
export function toPromptBlock(profile) {
    ...
    return [
        ...
        c.keyPhrases.length ? `**[Phrases họ hay dùng]** "${c.keyPhrases.slice(0, 5).join('", "')}"` : null,
        ...
        wb.expertiseSignals.length ? `**Expertise:** ${wb.expertiseSignals.slice(0, 5).join(', ')}` : null,
        wb.topicsOfInterest.length ? `**Topics:** ${wb.topicsOfInterest.slice(0, 5).join(', ')}` : null,
    ]
        .filter((l) => l != null)
        .join('\n');
}
