...
const imapConstructor = vi.fn(function() { return imap; }); // regular fn, not arrow
...
describe('send_email tool', () => {
  test('sends email with required fields', async () => {
    ...
    const result = JSON.parse(await sendEmailTool.func({
      to: 'recipient@example.com', subject: 'Test Subject', body: 'Hello',
    }));
  ...
  test('sends to multiple recipients (array)', async () => {
    setupSmtp();
    const result = JSON.parse(await sendEmailTool.func({
      to: ['a@test.com', 'b@test.com'], subject: 'Multi', body: 'Hi',
    }));
    expect(result.success).toBe(true);
  });
...
describe('reply_email tool', () => {
  test('sends reply with Re: prefix added', async () => {
    ...
    const result = JSON.parse(await replyEmailTool.func({
      replyToMessageId: '<orig@mail.com>', to: 'sender@example.com', subject: 'Hello', body: 'Got it!',
    }));
    expect(result.success).toBe(true);
  ...
  test('does not double-prefix Re:', async () => {
    ...
    await replyEmailTool.func({
      replyToMessageId: '<id@m.com>', to: 'a@t.com', subject: 'Re: Already', body: 'OK',
    });
...
describe('read_emails tool', () => {
  test('returns email list from INBOX', async () => {
    ...
    expect(result.data[0]).toMatchObject({
      uid: 101, subject: 'Test Email', from: 'sender@example.com', read: true,
    });
  });
    ...
    setupFetch([
      {
        ...
        envelope: {
          ...
          subject: 'Important', date: new Date(),
        },
        flags: new Set(),
      },
    ]);
...
describe('draft_email tool', () => {
  test('saves draft to \\Drafts IMAP folder', async () => {
    ...
    const result = JSON.parse(await draftEmailTool.func({
      to: 'someone@example.com', subject: 'Draft Subject', body: '<p>Draft</p>',
    }));
    expect(result.success).toBe(true);
  ...
  test('falls back to config.draftsFolder when no \\Drafts found', async () => {
    ...
    const result = JSON.parse(await draftEmailTool.func({
      to: 'a@b.com', subject: 'Sub', body: 'Body',
    }));
    expect(result.success).toBe(true);
...
describe('mark_email tool', () => {
  test.each([
    ...
    if (expectAdd)    expect(imap.messageFlagsAdd).toHaveBeenCalledWith(expect.anything(), expectAdd, expect.anything());
    if (expectRemove) expect(imap.messageFlagsRemove).toHaveBeenCalledWith(expect.anything(), expectRemove, expect.anything());
  });
