...
export const addExpenseTool = new DynamicStructuredTool({
    name: "add_expense",
    description:
        "Ghi lại chi tiêu mới vào sổ tài chính. Dùng khi người dùng muốn theo dõi chi tiêu.",
    schema: z.object({
...
export const checkExpenseTool = new DynamicStructuredTool({
    name: "check_expense",
    description:
        "Xem thống kê chi tiêu theo khoảng thời gian hoặc danh mục.",
    schema: z.object({
    ...
    func: async ({ startDate, endDate, category, currency }) => {
        ...
        try {
            ...
            const recent = filtered.slice(-10).map(([date, cat, desc, amount, cur, notes]) => ({
                date, category: cat, description: desc, amount: parseFloat(amount), currency: cur, notes,
            }));
