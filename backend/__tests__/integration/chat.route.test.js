...
describe('POST /api/chat/agent', () => {
    test('200 — returns answer and conversationId', async () => {
        ...
        const res = await supertest(app).post('/api/chat/agent').send({ query: 'Hello', conversationId: 'session-1' });
    ...
    test('200 — creates new conversationId when not provided', async () => {
        AgentService.chat.mockResolvedValueOnce([{ role: 'assistant', content: 'Hi there!' }]);

        const res = await supertest(app).post('/api/chat/agent').send({ query: 'Hello' });
    ...
    test('400 — missing query field', async () => {
        const res = await supertest(app).post('/api/chat/agent').send({});
    ...
    test('400 — empty query string', async () => {
        const res = await supertest(app).post('/api/chat/agent').send({ query: '' });
    ...
    test('500 — agent service throws returns 500', async () => {
        ...
        const res = await supertest(app).post('/api/chat/agent').send({ query: 'test', conversationId: 'err-session' });
    ...
    test('conversation history is appended for same session', async () => {
        ...
        AgentService.chat.mockResolvedValue([{ role: 'assistant', content: 'Response' }]);
...
describe('POST /api/chat', () => {
    test('200 — returns RAG answer with sources', async () => {
        ...
        const res = await supertest(app).post('/api/chat').send({ query: 'What is RAG?', strategy: 'traditional' });
