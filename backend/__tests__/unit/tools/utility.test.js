...
let translateTextTool, convertTimezoneTool, getCurrentTimeTool,
    generateQrCodeTool, shortenUrlTool, getExchangeRateTool, calculateTool;
let axiosMock;
...
describe('translate_text tool', () => {
  test('translates successfully', async () => {
    ...
    const result = JSON.parse(await translateTextTool.func({
  ...
  test('supports sourceLanguage override', async () => {
    ...
    const result = JSON.parse(await translateTextTool.func({
      text: 'Hello', targetLanguage: 'fr', sourceLanguage: 'en',
    }));
    expect(result.success).toBe(true);
...
describe('generate_qr_code tool', () => {
  test('generates QR for URL', async () => {
    const result = JSON.parse(await generateQrCodeTool.func({
...
describe('get_exchange_rate tool', () => {
  test('returns conversion rates', async () => {
    ...
    const result = JSON.parse(await getExchangeRateTool.func({
      baseCurrency: 'USD',
  ...
  test('API error returns fail', async () => {
    ...
    const result = JSON.parse(await getExchangeRateTool.func({
      baseCurrency: 'USD', targetCurrencies: ['VND'], amount: 1,
    }));
    expect(result.success).toBe(false);
  });
});
