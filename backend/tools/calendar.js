...
export const getEventTypesTool = new DynamicStructuredTool({
    ...
    description: 'Lấy danh sách các loại lịch hẹn (event types). Dùng khi cần chọn loại meeting phù hợp.',
...
export const createEventTypeTool = new DynamicStructuredTool({
    ...
    schema: z
        .object({
            ...
            locations: z
                ...
                .optional()
                .describe('Supported integrations via API: cal-video, google-meet, office365-video, zoom'),
    ...
    func: async (input) => {
        ...
        const allowedKeys = ['lengthInMinutes', 'title', 'slug', 'description', 'lengthInMinutesOptions'];

        try {
            const cleanInput = Object.fromEntries(Object.entries(input).filter(([k]) => allowedKeys.includes(k)));
            const res = await withRetry(() =>
                axios.post(`${config.cal.baseUrl}/event-types`, cleanInput, {
                    headers: {
                        ...getHeaders(),
                        'cal-api-version': '2024-06-14',
                    },
...
export const getCalendarEventsTool = new DynamicStructuredTool({
    ...
    func: async ({ timeMin, timeMax, maxResults }) => {
        ...
        try {
            ...
            const events = (res.data.bookings || []).slice(0, maxResults).map((b) => ({
                id: b.id,
...
const CreateBookingSchema = z
    .object({
        // Required
        start: z.string().describe('Thời gian bắt đầu ISO 8601 UTC, vd: 2026-04-20T09:00:00Z'),
        ...
        bookingFieldsResponses: z.record(z.any()).optional().describe('Giá trị cho custom booking fields'),
        metadata: z.record(z.string()).optional().describe('Metadata tuỳ chỉnh (<=50 keys)'),
        lengthInMinutes: z.number().int().optional().describe('Độ dài booking nếu event type có nhiều lựa chọn'),
...
export const createCalendarEventTool = new DynamicStructuredTool({
    ...
    func: async (input) => {
        ...
        const cleanInput = Object.fromEntries(Object.entries(input).filter(([k]) => k !== 'timestamp'));

        try {
            const res = await withRetry(() =>
                axios.post(`${config.cal.baseUrl}/bookings`, cleanInput, {
                    headers: {
                        ...getHeaders(),
...
export const deleteCalendarEventTool = new DynamicStructuredTool({
    ...
    func: async ({ eventId }) => {
        ...
        try {
            await withRetry(() =>
                axios.post(`${config.cal.baseUrl}/bookings/cancel`, { bookingId: eventId }, { headers: getHeaders() }),
            );
...
export const checkAvailabilityTool = new DynamicStructuredTool({
    ...
    description:
        'Kiểm tra slot trống từ Cal.com. Nếu eventTypeId chưa có hãy gọi đến các tool như get_event_types, create_event_types để lấy',
    schema: z
        .object({
