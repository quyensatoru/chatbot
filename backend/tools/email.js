        ...
        try {
            ...
            if (isAuthError || i === config.defaults.retryAttempts - 1) {
                if (isAuthError && config.email.smtpHost.includes("gmail.com")) {
                    err.message += " | Gmail yêu cầu App Password (không phải mật khẩu thường): myaccount.google.com/apppasswords";
                }
                throw err;
            }
...
export const markEmailTool = new DynamicStructuredTool({
    ...
    schema: z.object({
        ...
        action: z.enum(["read", "unread", "flag", "unflag", "delete"]).describe(
            "read=đã đọc, unread=chưa đọc, flag=quan trọng, unflag=bỏ quan trọng, delete=xóa"
        ),
    }).strict(),
