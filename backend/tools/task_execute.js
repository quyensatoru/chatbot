...
export const createTaskTool = new DynamicStructuredTool({
    name: 'create_task',
    description: 'Tạo task/công việc mới trong Notion. Dùng khi người dùng muốn ghi lại việc cần làm, đặt deadline.',
    schema: z
        .object({
...
export const listTasksTool = new DynamicStructuredTool({
    name: 'list_tasks',
    description: 'Liệt kê danh sách công việc từ Notion. Có thể lọc theo trạng thái, độ ưu tiên, hoặc tag.',
    schema: z
        .object({
...
export const updateTaskTool = new DynamicStructuredTool({
    ...
    func: async ({ taskId, title, status, priority, dueDate, notes }) => {
        ...
        try {
            ...
            const res = await withRetry(() => notion.pages.update({ page_id: taskId, properties: updates }));
            return ok(parseTask(res));
        } catch (err) {
...
export const deleteTaskTool = new DynamicStructuredTool({
    ...
    func: async ({ taskId }) => {
        ...
        try {
            const notion = getNotion();
            await withRetry(() => notion.pages.update({ page_id: taskId, archived: true }));
