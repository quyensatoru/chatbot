...
describe('email tools', () => {
  describe('send_email', () => {
    test('sends with required fields', async () => {
      ...
      const result = JSON.parse(await sendEmailTool.func({
    ...
    test('SMTP error returns fail', async () => {
      ...
      const result = JSON.parse(await sendEmailTool.func({
        to: 'a@test.com', subject: 'Sub', body: 'Body',
      }));
      expect(result.success).toBe(false);
    });
  });
    ...
    test('returns emails list', async () => {
      ImapFlow.mockImplementation(() => ({
        ...
        fetch: jest.fn().mockReturnValue((async function* () {
          yield { envelope: { subject: 'Test', from: [{ address: 'a@b.com' }], date: new Date() }, uid: 1 };
        })()),
        logout: jest.fn(),
      }));
  ...
  describe('search_emails', () => {
    test('searches by keyword', async () => { /* similar mock pattern */ });
...
describe('calendar tools', () => {
  describe('get_event_types', () => {
    test('returns event types list', async () => {
      nock(CAL_BASE).get('/v1/event-types').query(true).reply(200, {
  ...
  describe('book_event', () => {
    test('books successfully with valid input', async () => {
      ...
      const result = JSON.parse(await bookEventTool.func({
        eventTypeId: 1,
    ...
    test('returns fail for past datetime', async () => {
      ...
      const result = JSON.parse(await bookEventTool.func({
        eventTypeId: 1, start: '2020-01-01T09:00:00Z',
  ...
  describe('cancel_booking', () => {
    test('cancels with reason', async () => {
      ...
      const result = JSON.parse(await cancelBookingTool.func({
        uid: 'uid-123', reason: 'Meeting rescheduled',
      }));
      expect(result.success).toBe(true);
    });
  });
  ...
  describe('check_availability', () => {
    test('returns available slots', async () => {
      nock(CAL_BASE).get('/v1/slots').query(true).reply(200, {
        slots: { '2026-04-21': [{ time: '09:00' }, { time: '10:00' }] },
      });
      const result = JSON.parse(await checkAvailabilityTool.func({
        eventTypeId: 1,
...
describe('task tools (Notion)', () => {
  describe('create_task', () => {
    test('creates task with priority and deadline', async () => {
      ...
      const result = JSON.parse(await createTaskTool.func({
  ...
  describe('update_task', () => {
    test('updates task status to done', async () => {
      ...
      const result = JSON.parse(await updateTaskTool.func({
        taskId: 'page-id-123', status: 'done',
      }));
      expect(result.success).toBe(true);
    });
  });
...
describe('file tools', () => {
  ...
  describe('write_file', () => {
    ...
    test('overwrites existing file with confirm flag', async () => {
      fs.writeFile.mockResolvedValue(undefined);
      const result = JSON.parse(await writeFileTool.func({
        path: 'existing.txt', content: 'Updated', overwrite: true,
      }));
      expect(result.success).toBe(true);
    });
  });
...
describe('automation tools', () => {
  describe('create_automation', () => {
    ...
    test('rejects invalid cron expression', async () => {
      ...
      const result = JSON.parse(await createAutomationTool.func({
        name: 'Bad cron', schedule: 'not-a-cron', action: 'send_message', actionParams: {},
      }));
      expect(result.success).toBe(false);
    });
  });
  ...
  describe('persistence — recovery on restart', () => {
    test('automations are saved to automations.json', async () => {
      ...
      await createAutomationTool.func({
        name: 'Test', schedule: '* * * * *', action: 'test', actionParams: {},
      });
...
describe('message_chat tools', () => {
  ...
  describe('send_message', () => {
    test('sends to channel with markdown', async () => {
      ...
      const result = JSON.parse(await sendMessageTool.func({
        channel: 'general',
        message: '**Hello World**',
      }));
      expect(result.success).toBe(true);
  ...
  describe('schedule_message', () => {
    test('schedules message for future time', async () => {
      const result = JSON.parse(await scheduleMessageTool.func({
        channel: 'general',
        message: 'Reminder!',
...
describe('finance tools', () => {
  describe('add_expense', () => {
    test('appends expense row to sheet', async () => {
      ...
      const result = JSON.parse(await addExpenseTool.func({
        amount: 150000,
        category: 'Food',
        description: 'Lunch',
        date: '2026-04-20',
    ...
    test('negative amount returns fail', async () => {
      const result = JSON.parse(await addExpenseTool.func({
        amount: -100, category: 'Test', description: 'Test', date: '2026-04-20',
      }));
      expect(result.success).toBe(false);
    });
  });
...
describe('utility tools', () => {
  describe('translate_text', () => {
    test('translates Vietnamese to English', async () => {
      nock('https://translation.googleapis.com')
        .post(/./).reply(200, { data: { translations: [{ translatedText: 'Hello World' }] } });
      const result = JSON.parse(await translateTool.func({
        text: 'Xin chào thế giới', targetLanguage: 'en',
      }));
      expect(result.success).toBe(true);
  ...
  describe('convert_timezone', () => {
    test('converts Ho Chi Minh to UTC', async () => {
      const result = JSON.parse(await convertTimezoneTool.func({
...
describe('api_call tool', () => {
  test('GET request with query params', async () => {
    ...
    const result = JSON.parse(await apiCallTool.func({
      method: 'GET', url: 'https://api.example.com/users', params: { page: 1 },
    }));
    expect(result.success).toBe(true);
  });
  ...
  test('custom headers are sent', async () => {
    nock('https://api.example.com')
      .get('/protected').matchHeader('Authorization', 'Bearer token123').reply(200, { ok: true });
    const result = JSON.parse(await apiCallTool.func({
      method: 'GET', url: 'https://api.example.com/protected',
...
describe('document_rag tool', () => {
  test('queries RAG service and returns answer', async () => {
    ...
    const result = JSON.parse(await documentRagTool.func({
...
describe('POST /api/chat/agent', () => {
  ...
  test('200 — basic query returns answer', async () => {
    const res = await request(app)
      .post('/api/chat/agent')
      .send({ query: 'Xin chào', conversationId: 'session-1' });
  ...
  test('conversation window caps at 10 messages', async () => {
    ...
    for (let i = 0; i < 12; i++) {
      await request(app).post('/api/chat/agent').send({ query: `Message ${i}`, conversationId: convId });
    }
...
describe('Agent Tool Selection', () => {
  let agent;
  beforeAll(() => { agent = new AgentService(); });
...
describe('Tool Schema Validation (Zod)', () => {
  ...
  test.each(invalidInputCases)(
    '%s rejects invalid input: %j',
    async (toolName, input, errorFragment) => {
