...
vi.mock('../../../config/llm.config.js', () => ({ openChatModel: { pipe: vi.fn().mockReturnThis(), invoke: vi.fn() } }));
...
describe('All tools — Zod schema validation', () => {
  ...
  test.each(invalidInputCases)(
    'tool "%s" rejects invalid input via schema: %s',
    (toolName, invalidInput, reason) => {
...
describe('Tool registry completeness', () => {
  const expectedTools = [
    'calculate_math', 'current_time',
    'web_search', 'api_call',
    'send_email', 'reply_email', 'read_emails', 'search_emails', 'draft_email', 'mark_email',
    'list_files', 'read_file', 'create_file', 'write_file', 'delete_file', 'search_files',
    'create_automation', 'list_automations', 'toggle_automation', 'delete_automation',
    'translate_text', 'calculate', 'convert_timezone', 'get_current_time',
    'generate_qr_code', 'shorten_url', 'get_exchange_rate',
  ];
